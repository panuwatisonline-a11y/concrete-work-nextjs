...
function Field({ label, value }: { label: string; value: React.ReactNode }) {
  ...
  return (
    <div>
      <p className="text-[10px] uppercase tracking-widest text-[--text-faint] mb-0.5">{label}</p>
  ...
  return (
    <Card className="overflow-hidden">
      <div className="px-4 py-2.5 border-b border-[--border-subtle]">
        <p className="text-[10px] font-semibold text-[--text-faint] uppercase tracking-widest">{title}</p>
      </div>
    ...
    <main className="min-h-screen">
      <header className="border-b border-[--border] bg-[--surface] sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center gap-3">
          <Link
            href={req.status_id ? `/requests?status=${req.status_id}` : "/"}
            className="w-8 h-8 flex items-center justify-center rounded-[--radius-sm] hover:bg-[--border-subtle] transition shrink-0"
          >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-[--text] truncate">{req.full_location ?? "รายละเอียดคำขอ"}</p>
            <p className="text-xs text-[--text-faint] mt-0 truncate">
              {formatDate(req.request_date)}{req.request_time && ` · ${req.request_time.slice(0, 5)}`}
            </p>
          </div>
          <div className="shrink-0"><StatusBadge statusId={req.status_id} statusName={req.status_name} /></div>
        </div>
      </header>

      <div className="max-w-2xl mx-auto px-4 py-5 space-y-3">
        <Section title="ข้อมูลการขอ">
          <Field label="วันที่ขอ" value={<>{formatDate(req.request_date)}{req.request_time && <span className="ml-1.5 text-[--text-faint]">{req.request_time.slice(0, 5)}</span>}</>} />
          <Field label="วันที่เทคอนกรีต" value={formatDate(req.casting_date)} />
          <Field label="ผู้ว่าจ้าง" value={req.client_name} />
          <FullWidthField label="สถานที่" value={req.full_location} />
          <Field label="โครงสร้าง" value={req.structure_name} />
          <Field label="หมายเลขโครงสร้าง" value={req.structure_no} />
          <FullWidthField label="ประเภทงาน" value={req.concrete_work} />
        </Section>

        <Section title="ข้อมูลคอนกรีต">
          <Field label="Mix Code" value={req.mixcode} />
          ...
          <Field label="ปริมาตรที่ขอ (m³)" value={<span className="font-semibold">{req.volume_request?.toLocaleString() ?? "-"}</span>} />
        ...
        {(req.full_wbs || req.full_abc) && (
          <Section title="รหัสโปรเจกต์">
            <FullWidthField label="WBS Code" value={req.full_wbs} />
            <FullWidthField label="ABC Code" value={req.full_abc} />
          </Section>
        )}

        {req.postpone_date && (
          <Section title="ข้อมูลการเลื่อน">
            <Field label="วันที่เลื่อนเป็น" value={<>{formatDate(req.postpone_date)}{req.postpone_time && <span className="ml-1.5 text-[--text-faint]">{req.postpone_time.slice(0, 5)}</span>}</>} />
            <FullWidthField label="เหตุผล" value={req.reason_postpone} />
          </Section>
        )}

        {(req.reason_reject || req.reason_cancel) && (
          <Section title="เหตุผล">
            <FullWidthField label="เหตุผลการปฏิเสธ" value={req.reason_reject} />
            <FullWidthField label="เหตุผลการยกเลิก" value={req.reason_cancel} />
          </Section>
        )}

        {req.remarks && (
          <Card className="px-4 py-4">
            <p className="text-[10px] uppercase tracking-widest text-[--text-faint] mb-1.5">หมายเหตุ</p>
            <p className="text-sm text-[--text-2] leading-relaxed">{req.remarks}</p>
          </Card>
        )}
        ...
        {timeline.length > 0 && (
          <Card className="overflow-hidden">
            <div className="px-4 py-2.5 border-b border-[--border-subtle]">
              <p className="text-[10px] font-semibold text-[--text-faint] uppercase tracking-widest">ประวัติการดำเนินการ</p>
            </div>
